      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-100 bg-white sticky top-0 z-10">
          <div 
            ...
            className="flex items-center gap-2 cursor-pointer group select-none active:opacity-70"
            title={t('start_new_search')}
          >
            <div className="bg-[#005993] p-1.5 rounded-lg active:scale-95 transition-transform">
              <LayoutGrid className="w-5 h-5 text-white" />
            </div>
            <span className="font-bold text-[#005993] text-lg">{t('menu')}</span>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-grow overflow-y-auto">
          ...
          {count > 0 && (
            <div className="p-5 border-b border-slate-100 animate-in slide-in-from-left-2 duration-300">
              ...
              >
                <Plus className="w-4 h-4" /> {t('start_new_search')}
              </button>

              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">{t('current_session')}</h3>
              
              <div className="space-y-4">
                {/* Stats */}
                <div className="flex items-center justify-between bg-[#f0f9ff] p-3 rounded-xl border border-[#7ED3F7]/30">
                  <span className="text-sm font-medium text-[#005993]">{t('results_found')}</span>
                  <span className="text-xl font-bold text-[#005993]">{count}</span>
                </div>

                {/* View Mode */}
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-slate-600">{t('view_layout')}</label>
                ...
                {/* Sort */}
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-slate-600">{t('sort_by')}</label>
                  <select 
                    value={sortBy}
                  ...
                  >
                    <option value={SortOption.RATING_DESC}>{t('sort_rating')}</option>
                    <option value={SortOption.REVIEWS_DESC}>{t('sort_reviews')}</option>
                    <option value={SortOption.NAME_ASC}>{t('sort_name')}</option>
                  </select>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <button onClick={onOpenAnalytics} className="flex flex-col items-center justify-center p-3 bg-slate-50 border border-slate-200 rounded-xl active:scale-95 transition-all active:bg-blue-50">
                    ...
                    <span className="text-xs font-bold text-slate-600">{t('analysis')}</span>
                  </button>
                  <button onClick={onDownloadExcel} className="flex flex-col items-center justify-center p-3 bg-slate-50 border border-slate-200 rounded-xl active:scale-95 transition-all active:bg-green-50">
                    ...
                    <span className="text-xs font-bold text-slate-600">{t('excel')}</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="p-5">
            ...
            {history.length === 0 ? (
              <div className="text-center py-8 text-slate-400 bg-slate-50 rounded-xl border border-slate-100 border-dashed">
                ...
                <p className="text-xs font-medium">{t('no_history')}</p>
              </div>
            ) : (
              <div className="space-y-3">
                {history.map((item) => (
                  <div 
                    ...
                    className="group bg-white border border-slate-100 rounded-xl p-3 md:hover:border-[#7ED3F7] active:bg-[#f0f9ff] active:border-[#7ED3F7] transition-all cursor-pointer shadow-sm relative active:scale-[0.99]"
                  >
                    <div className="flex justify-between items-start gap-2">
                        ...
                        <div className="flex flex-col items-end gap-2 shrink-0 z-10">
                            <button
                                ...
                                className="relative p-2 -mr-2 -mt-2 text-slate-400 md:hover:text-[#D71249] md:hover:bg-red-50 active:text-[#D71249] active:bg-red-50 rounded-lg transition-colors z-20"
